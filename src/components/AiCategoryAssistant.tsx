...
export function AiCategoryAssistant({
  isOpen,
  onClose,
}: {
  isOpen: boolean;
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Hi! Tell me what your business does, and I'll tell you exactly which category to select.",
    },
  ]);
      ...
      if (data.success) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              "Oops, my network dropped. Can you repeat that?",
          },
        ]);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`relative w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col h-[85dvh] sm:h-[550px] transition-transform duration-300 ${
          isOpen
      ...
      >
        ...
        <div
          ...
          className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-200 overscroll-contain touch-pan-y"
          style={{
          ...
          {isTyping && (
            <div className="flex gap-3">
              ...
              <div className="px-5 py-4 rounded-2xl rounded-tl-sm bg-slate-50 border border-slate-100 flex items-center gap-1.5 w-fit shadow-sm">
                <span className="h-1.5 w-1.5 bg-[#ff3f7a] rounded-full animate-bounce [animation-delay:-0.3s]" />
                <span className="h-1.5 w-1.5 bg-[#ff3f7a] rounded-full animate-bounce [animation-delay:-0.15s]" />
                <span className="h-1.5 w-1.5 bg-[#ff3f7a] rounded-full animate-bounce" />
              </div>
            </div>
          )}
        </div>
