...
import { X, User, ShieldAlert, Wallet, Receipt, FileText, CheckCircle2, AlertCircle, TrendingUp, TrendingDown, RefreshCw } from 'lucide-react';
      ...
      <div className="relative w-full max-w-2xl h-full bg-white dark:bg-zinc-950 border-l border-zinc-200 dark:border-zinc-800 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 shrink-0">
          ...
          <div className="flex justify-between items-start">
            <div>
              <h4 className="text-xl font-bold text-zinc-900 dark:text-white leading-tight">{client.firstName} {client.lastName}</h4>
          ...
          <div className="flex gap-4 mt-6 border-b border-zinc-200 dark:border-zinc-800 overflow-x-auto scrollbar-hide">
            <TabBtn label="Bio & Security" active={activeTab === "BIO"} onClick={() => setActiveTab("BIO")} />
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-zinc-50 dark:bg-zinc-950/50">
          ...
          {activeTab === "BIO" && (
            <div className="space-y-6 animate-in fade-in">
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-xl p-5 border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <h4 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-4 border-b border-zinc-100 dark:border-zinc-800 pb-2 flex items-center"><ShieldAlert size={16} className="mr-2 text-indigo-500"/> Security Settings</h4>
                <div className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg border border-zinc-100 dark:border-zinc-700/50">
                  <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Two-Factor Authentication (2FA)</span>
                  <span className={`text-xs font-bold px-2 py-1 rounded ${client.twoFactorEnabled ? 'bg-emerald-100 text-emerald-700' : 'bg-zinc-200 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400'}`}>
            ...
            <div className="space-y-4 animate-in fade-in">
              {client.transactions?.length === 0 ? (
                ...
                client.transactions?.map((tx: any) => (
                  <div key={tx.id} className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 flex justify-between items-center shadow-sm">
                    <div className="flex items-center">
                      <div className={`p-2 rounded-lg mr-3 ${tx.type === 'CREDIT' || tx.type === 'REFUND' || tx.type === 'ADJUSTMENT' ? 'bg-emerald-100 text-emerald-600' : 'bg-zinc-100 text-zinc-600 dark:bg-zinc-800'}`}>
                        {tx.type === 'CREDIT' || tx.type === 'REFUND' || tx.type === 'ADJUSTMENT' ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{tx.description}</p>
                        <p className="text-xs text-zinc-500">{format(new Date(tx.createdAt), 'MMM d, h:mm a')} • {tx.reference}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <span className="text-[10px] uppercase font-bold text-zinc-400">{tx.status}</span>
                    </div>
                  </div>
                ))
              )}
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              {[...client.registrations, ...client.llcRegistrations].length === 0 ? (
                ...
                [...client.registrations, ...client.llcRegistrations].map((reg: any) => (
                  <div key={reg.id} className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 flex justify-between items-center shadow-sm">
                    <div>
                      <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{reg.proposedName}</p>
                      <p className="text-xs text-zinc-500 font-mono">{reg.trackingId || 'Draft'}</p>
                    </div>
                    <span className="text-xs font-bold px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded">{reg.status}</span>
                  </div>
                ))
              )}
              ...
              {client.ninRequests?.length === 0 ? (
                ...
                client.ninRequests?.map((nin: any) => (
                  <div key={nin.id} className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 flex justify-between items-center shadow-sm">
                    <div>
                      <p className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{nin.slipType.replace('_', ' ').toUpperCase()}</p>
                      <p className="text-xs text-zinc-500 font-mono">{nin.ninMasked}</p>
                    </div>
                    <span className="text-xs font-bold px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded">{nin.status}</span>
                  </div>
                ))
              )}
            </div>
          ...
          {activeTab === "ACTIONS" && (
            <div className="space-y-8 animate-in fade-in">
              ...
              {/* Wallet Adjustment */}
              <div className="bg-white dark:bg-zinc-900 p-5 rounded-xl border-2 border-indigo-100 dark:border-indigo-900/30 shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 pt-2">
                    <button onClick={() => handleAction("CREDIT_WALLET", { amount: adjustAmount, reason: adjustReason })} disabled={isProcessing} className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-xs transition-colors">Credit Wallet (+)</button>
                    <button onClick={() => handleAction("DEBIT_WALLET", { amount: adjustAmount, reason: adjustReason })} disabled={isProcessing} className="flex-1 py-2 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded-lg text-xs transition-colors">Debit Wallet (-)</button>
                  </div>
                </div>
              </div>

              {/* Suspension */}
              <div className="bg-red-50 dark:bg-red-500/5 p-5 rounded-xl border-2 border-dashed border-red-200 dark:border-red-500/30">
              ...
              {error && <p className="text-xs text-red-600 font-bold text-center bg-red-50 py-2 rounded">{error}</p>}
            </div>
          )}
        </div>
      </div>
...
function TabBtn({ label, active, onClick, danger }: any) {
  return (
    <button onClick={onClick} className={`whitespace-nowrap pb-2 text-sm font-medium border-b-2 transition-colors ${active ? (danger ? 'border-red-500 text-red-600' : 'border-indigo-500 text-indigo-600') : 'border-transparent text-zinc-500 hover:text-zinc-900'}`}>
      {label}
    </button>
  );
}
