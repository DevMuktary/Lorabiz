...
export type ModificationStatusFilter = "ALL" | "PENDING" | "PROCESSING" | "COMPLETED" | "REJECTED";

interface ModificationHistoryStatsProps {
  stats: {
    total: number;
    pending: number;
    processing: number;
  ...
  activeFilter?: ModificationStatusFilter;
  onFilterChange?: (filter: ModificationStatusFilter) => void;
}

export function ModificationHistoryStats({
  stats,
  activeFilter = "ALL",
  onFilterChange,
}: ModificationHistoryStatsProps) {
  const statCards = [
    {
      id: "ALL" as const,
