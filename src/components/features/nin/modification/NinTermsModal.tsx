...
export function NinTermsModal({ isOpen, userFullName = "", onAgreed }: NinTermsModalProps) {
  ...
  useEffect(() => {
    ...
    // Allow layout to settle before sizing canvas
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Smooth quadratic curve midpoint interpolation for round curves & cursive signatures
    const midX = (prevPt.x + currentPt.x) / 2;
    const midY = (prevPt.y + currentPt.y) / 2;
  ...
  return (
    <div className="w-full bg-card border border-border shadow-xl rounded-3xl p-6 sm:p-8 space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center gap-3.5 pb-5 border-b border-border">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Review the terms of agreement and provide your digital authorization to unlock the modification service.
          </p>
        </div>
      </div>

      {/* Prominent Scroll Down Indicator Pill */}
      <div className="p-3.5 rounded-2xl bg-secondary/60 border border-border flex items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2 text-foreground font-medium">
          <span className="flex h-2 w-2 rounded-full bg-primary animate-pulse shrink-0" />
          <span>Please review all clauses below and sign to proceed</span>
        </div>
        <div className="flex items-center gap-1 font-bold text-primary animate-bounce text-[11px] shrink-0">
          <span>Scroll down to sign</span>
      ...
      <form onSubmit={handleSubmit} className="pt-5 space-y-5 border-t border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-background border border-border text-foreground text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary transition-all"
              required
            />
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1 bg-secondary p-0.5 rounded-lg text-[10px] font-bold border border-border">
                <button
                  ...
                  className={`px-2.5 py-0.5 rounded-md transition-all cursor-pointer ${signatureMode === "draw" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
                >
                  Draw
                </button>
                <button
                  ...
                  className={`px-2.5 py-0.5 rounded-md transition-all cursor-pointer ${signatureMode === "type" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
                >
                  Type
                </button>
              </div>
            ...
            {signatureMode === "draw" ? (
              <div className="relative">
                <canvas
                  ...
                  className="w-full h-28 sm:h-32 rounded-xl border border-border bg-white cursor-crosshair shadow-inner"
                />
                {hasDrawn && (
                  <button
                    type="button"
              ...
              <div>
                <input
                  ...
                  className="w-full h-28 sm:h-32 px-4 py-2 rounded-xl bg-white border border-border text-slate-900 font-serif italic text-xl sm:text-2xl flex items-center justify-center text-center shadow-inner focus:outline-none focus:ring-2 focus:ring-primary"
                />
              </div>
            )}
          </div>
