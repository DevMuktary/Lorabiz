      ...
      <form onSubmit={handleFormSubmit} className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
        ...
        <div className="pt-4 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-2.5 sm:py-3 rounded-xl bg-primary text-primary-foreground font-bold text-xs sm:text-sm shadow-md hover:opacity-90 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
            <ShieldCheck weight="bold" className="h-4 w-4" />
            {isPassApplied ? (
              <span>Submit (₦0.00 Free)</span>
            ) : (
              <span>Proceed · ₦{servicePrice.toLocaleString()}</span>
            )}
          </button>
        </div>
      </form>
