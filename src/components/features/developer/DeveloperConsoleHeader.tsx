...
export const DeveloperConsoleHeader: React.FC<DeveloperConsoleHeaderProps> = ({
  ...
  const navLinks = [
    ...
    },
    {
      ...
      active: pathname === "/dashboard/developer/requests",
    },
    {
      name: "API Transactions",
      href: "/dashboard/developer/transactions",
      active: pathname === "/dashboard/developer/transactions",
    },
    {
      name: "API Pricing",
      href: "/dashboard/developer/pricing",
      active: pathname.startsWith("/dashboard/developer/pricing"),
    },
  ];
    ...
    <div className="flex flex-col gap-4 border-b border-border/60 pb-4">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* Switch Toggle if onToggleEnvironment is provided */}
          {onToggleEnvironment ? (
            <div className="flex items-center gap-2.5 rounded-2xl border border-border/80 bg-muted/40 px-3.5 py-2">
              <span
                className={`text-xs font-semibold transition-colors ${
                  !isLive ? "text-amber-600 dark:text-amber-400 font-bold" : "text-muted-foreground"
                }`}
              >
                Test
              </span>
              ...
              <span
                className={`text-xs font-semibold transition-colors ${
                  isLive ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-muted-foreground"
                }`}
              >
                Live
              </span>
            ...
            <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 px-3.5 py-2 text-xs font-bold text-emerald-600 dark:text-emerald-400">
      ...
      <div className="flex items-center gap-1.5 border-t border-border/40 pt-3 overflow-x-auto">
        {navLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
