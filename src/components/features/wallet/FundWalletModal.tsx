...
export default function FundWalletModal({ isOpen, onClose, onSuccess, onFailure }: FundWalletModalProps) {
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 dark:bg-black/80 backdrop-blur-sm p-4 animate-in fade-in overflow-y-auto">
        <div 
        ...
        />
        <div className="relative bg-card text-card-foreground rounded-3xl p-6 sm:p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200 border border-border max-h-[90dvh] overflow-y-auto custom-scrollbar my-auto">
          ...
          <div className="space-y-6">
                ...
                <button
                  ...
                  className={`py-2.5 rounded-xl text-sm font-bold border-2 transition-all cursor-pointer disabled:opacity-50 ${
                    amount === amt.toString() 
                      ...
                      : "border-border text-foreground hover:border-primary/50 bg-secondary/50"
                  }`}
                >
                  ₦{amt.toLocaleString()}
                </button>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-9 pr-4 h-14 text-lg font-bold border-2 border-border bg-secondary/40 rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all disabled:opacity-50"
                />
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full h-14 bg-primary hover:opacity-90 text-primary-foreground font-black text-base rounded-xl flex items-center justify-center gap-2 shadow-xl shadow-primary/25 transition-all active:scale-[0.98] disabled:opacity-50 cursor-pointer"
            >
              {isProcessing ? (
                <>
