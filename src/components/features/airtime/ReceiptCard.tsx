...
export default function ReceiptCard({ transaction, onNewTransaction }: ReceiptCardProps) {
  ...
  return (
    <>
      ...
      <div id="printable-receipt" className="bg-card border border-border p-1 rounded-3xl shadow-xl max-w-sm mx-auto animate-in zoom-in-95 duration-500">
        <div className="bg-secondary/20 p-6 rounded-[22px] border border-dashed border-border flex flex-col items-center">
          ...
          <h3 className="font-black text-2xl text-foreground mb-1 text-center">Transaction Successful</h3>
          <p className="text-muted-foreground text-sm font-medium mb-6 text-center">Your airtime has been delivered.</p>
          ...
          <div className="w-full bg-background rounded-2xl p-5 shadow-sm space-y-4 relative border border-border">
            ...
            <div className="flex justify-between items-center">
              ...
              <Image src={logoMap[transaction.network] || "/mtn.png"} alt={transaction.network} width={30} height={30} className="object-contain" />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 w-full mt-6 no-print">
            <Button onClick={handleSave} variant="outline" className="w-full font-bold border-border shadow-sm flex gap-2 cursor-pointer">
              <DownloadSimple weight="bold" /> Save
            </Button>
            <Button onClick={handleShare} variant="outline" className="w-full font-bold border-border shadow-sm flex gap-2 cursor-pointer">
              <ShareNetwork weight="bold" /> Share
            </Button>
          </div>

          <Button onClick={onNewTransaction} className="w-full mt-3 font-black h-12 rounded-xl flex gap-2 bg-primary hover:opacity-90 text-primary-foreground cursor-pointer no-print">
