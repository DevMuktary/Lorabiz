...
export async function POST(req: NextRequest) {
  ...
  if (keyPayload.type === ApiKeyType.TEST) {
    ...
    if (sanitizedTrackingId === "0TDUPCONFLICT01") {
      const conflictMsg = `An active personalization request is already in progress for Tracking ID ${sanitizedTrackingId}. Duplicate submission rejected to prevent double debits.`;
      recordApiRequestLog({
        userId: keyPayload.userId,
        apiKeyId: keyPayload.id,
        environment,
    ...
    setTimeout(async () => {
      try {
        if (sanitizedTrackingId === "0TBH26SQHQCR9F") {
