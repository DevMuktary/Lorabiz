...
import { sendDocumentGeneratedEmail, sendScumlSubmittedEmail } from "@/lib/email";
import { generateAIBoardResolution, generateDeterministicResolution } from "@/lib/board-resolution-generator";

async function verifyPaymentTransaction(reference: string, req: Request) {
  ...
  if (!reference || typeof reference !== "string" || !/^[a-zA-Z0-9_-]+$/.test(reference)) {
    return NextResponse.json({ success: false, message: "Invalid transaction reference format" }, { status: 400 });
  }

  if (!reference.startsWith("ONL_") && !reference.startsWith("FW_") && !reference.startsWith("NSUB_")) {
    return NextResponse.json({ success: false, message: "Invalid transaction type for this endpoint" }, { status: 400 });
  }
  ...
  let createdDocumentRecord: any = null;
  ...
  await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    if (existingTx && existingTx.status === "SUCCESS") {
      if (isDoc) {
        createdDocumentRecord = await tx.generatedDocument.findFirst({
          where: { transactionRef: reference, userId: user.id }
        });
      }
      return; 
    }
    ...
    // =====================================================================
    // SCENARIO B: SERVICE CHECKOUT (CAC/SCUML/DOCUMENTS)
    // =====================================================================
    if (reference.startsWith("ONL_")) {
      ...
      if (isDoc && docDraft) {
        ...
        createdDocumentRecord = await tx.generatedDocument.create({
          data: {
            userId: user.id,
        ...
        const isLLC = await tx.llcRegistration.findUnique({ where: { id: registrationId } });
        if (isLLC) {
  ...
  // 3. BACKGROUND NOTIFICATIONS & EMAILS
  if (isDoc && docDraft) {
    try {
      ...
      await sendDocumentGeneratedEmail({
        ...
        documentId: registrationId || reference,
      });
    } catch (docEmailErr) {
      console.error("Failed to send Document Generated Email via verify route:", docEmailErr);
    }
  } else if (isScuml && scumlDraft) {
    try {
      const user = await prisma.user.findUnique({ where: { email: userEmail } });
      await sendScumlSubmittedEmail({
        ...
        companyName: scumlDraft.companyName,
        regType: scumlDraft.type || "Registration",
        transactionRef: reference
      });
    } catch (scumlEmailErr) {
      console.error("Failed to send SCUML Submitted Email via verify route:", scumlEmailErr);
    }
  } else if (reference.startsWith("ONL_") && !isDoc) {
    try {
      const user = await prisma.user.findUnique({ where: { email: userEmail } });
      if (user) {
        let proposedName = "Business Registration";
        const llc = await prisma.llcRegistration.findUnique({ where: { id: registrationId } });
        const bn = await prisma.businessRegistration.findUnique({ where: { id: registrationId } });
        if (llc) proposedName = llc.proposedName || "LLC Application";
        else if (bn) proposedName = bn.proposedName || "Business Name Application";
      ...
      console.error("Failed to enqueue registration submission notification:", regQueueErr);
    }
  }

  // 4. FIRST WALLET FUNDING EMAIL
  if (reference.startsWith("FW_")) {
  ...
  return NextResponse.json({ 
    ...
    document: createdDocumentRecord
  });
}
...
export async function POST(req: Request) {
  try {
    ...
    return await verifyPaymentTransaction(body?.reference, req);
  } catch (error: any) {
    console.error("Payment Verification Error (POST):", error.message);
...
export async function GET(req: Request) {
  try {
    ...
    return await verifyPaymentTransaction(reference || "", req);
  } catch (error: any) {
    console.error("Payment Verification Error (GET):", error.message);
    return NextResponse.json({ success: false, message: error.message || "Failed to process payment." }, { status: 500 });
  }
}
