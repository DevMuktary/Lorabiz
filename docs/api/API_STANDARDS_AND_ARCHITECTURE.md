...
| **`tracking_id`** | `string` | **Official NIMC Tracking ID**. Strictly reserved for the government-issued National Identity Management Commission tracking number (e.g., NIMC IPE Clearance and Verification Slips). **NEVER returned or used in NIN Validation responses**, where the platform token is strictly `reference`. | `0TEB51VS5RES4ZZ`, `0TBH26SQHQCR9F` |
