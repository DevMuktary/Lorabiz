...
async function main() {
  ...
  for (const p of prices) {
    await prisma.servicePricing.upsert({
      ...
      update: { price: p.price, title: p.title },
  ...
  const ninPrices = [
    { slipType: "nin_regular", displayName: "Regular Official Slip", price: 500.00 },
    { slipType: "nin_standard", displayName: "Standard Biometric Slip", price: 700.00 },
    { slipType: "nin_premium", displayName: "Premium Card Layout", price: 1000.00 }
  ]

  for (const np of ninPrices) {
    await prisma.ninSlipPricing.upsert({
      where: { slipType: np.slipType },
      update: { displayName: np.displayName, price: np.price },
      create: { slipType: np.slipType, displayName: np.displayName, price: np.price },
    })
  }
